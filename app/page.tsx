    ...
    <main className="home-shell">
      <aside className="home-sidebar" aria-label="메인 메뉴">
        ...
        {isLoadingUser ? (
          ...
          <>
            <div className={`sidebar-user ${isUserInfoCollapsed ? "collapsed" : ""}`}>
              <div className="sidebar-user-head">
                ...
                <button
                  type="button"
                  onClick={() => setIsUserInfoCollapsed((collapsed) => !collapsed)}
                  aria-label={isUserInfoCollapsed ? "로그인 정보 펼치기" : "로그인 정보 접기"}
              ...
              {!isUserInfoCollapsed ? (
                <div className="sidebar-user-details">
                  ...
                  <div><span>성별:</span><strong>{genderLabel}</strong></div>
                  <div><span>서비스:</span><strong>{membership?.serviceName || "가입정보 없음"}</strong></div>
                  <div><span>요금제:</span><strong>{membership?.plan || "-"}</strong></div>
                </div>
              ) : null}
              {!isUserInfoCollapsed ? <Link href="/api/auth/logout?returnTo=/login">로그아웃</Link> : null}
            </div>
